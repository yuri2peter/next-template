...
async function saveFileFromLink(link: string, timeout = 10 * 1000) {
  ...
  try {
    ...
    const url = await saveDataToPublic(buffer, filename);
    return {
      url,
      ext,
      contentType,
...
async function saveFileFromDataUrl(
  dataUrl: string // like: data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/x+AAwMCAO+ip1sAAAAASUVORK5CYII=
) {
  ...
  return {
    url,
    ext: fileEnding,
    contentType: mime.getType(fileEnding),
    size: buffer.length,
    filename,
  };
}
