...
import type { Attrs, Node, NodeType } from '@milkdown/kit/prose/model';
...
export function addNode(tr: Transaction, node: Node) {
  if (!node) return null;
...
export function clearContentAndAddNode(node: Node): Command {
  return (state, dispatch) => {
    const tr = addNode(clearRange(state.tr), node);
    if (!tr) return false;
